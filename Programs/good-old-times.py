...
# (PART 2) find the 50 most frequent words in secondary memory
...
# PART 2
# Now we need to find the 50 most frequently occurring words.
# We don't need anything from the previous values in memory
del data[:]

# Let's use the first 50 entries for the top 50 words
data = data + [[]] * (50 - len(data))
data.append('')  # data[50] is word,freq from file
data.append(0)  # data[26] is freq

# Loop over secondary memory file
while True:
    data[50] = str(word_freqs.readline().strip(), 'utf-8')
    if data[50] == '':  # EOF
        break
    data[51] = int(data[50].split(',')[1])  # Read it as integer
    data[50] = data[50].split(',')[0].strip()  # word
    # Check if this word has more counts than the ones in memory
    for i in range(50):  # elimination of symbol i is exercise
        if data[i] == [] or data[i][1] < data[51]:
            data.insert(i, [data[50], data[51]])
            del data[51]  # delete the last element
            break

for tf in data[0:50]:  # elimination of symbol tf is exercise
    if len(tf) == 2:
        print(tf[0], '-', tf[1])
# We're done
word_freqs.close()
